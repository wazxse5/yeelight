...
import wazxse5.UID
import wazxse5.cli.CLI._
import wazxse5.command.{SetBrightness, SetPower, SetRgb, YeelightCommand}
import wazxse5.model.IYeelightService
...
class CLI(service: IYeelightService) {
  ...
  private var cliDevices: MutableMap[UID, CliDeviceInfo] = MutableMap.empty
  ...
  private def findDeviceInternalId(identifier: String): Option[UID] = {
    ...
    lazy val asInternalId = cliDevices.collectFirst {
      case (uid, _) if uid.uid == identifier => uid
    }
  ...
  private def performDevice(deviceInternalId: UID, words: List[String]): Unit = if (words.nonEmpty) {
  ...
  private def setAlias(deviceInternalId: UID, alias: String): Unit = {
